...
export default function StudentLogin() {
  ...
  useEffect(() => {
    // Only redirect if already logged in as a student
    if (isReady && user && profile?.role === "student") {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error, roleMismatch, actualRole } = await signInWithRole(email, password, "student");

      if (roleMismatch) {
        // Role doesn't match - block login with clear message
        const roleLabel = actualRole === "admin" ? "Admin" : actualRole;
        toast({
          title: "Access Denied",
